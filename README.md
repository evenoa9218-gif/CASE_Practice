...
- **사례집 목차로 풀기** (기본) — 강성민 헌법·행정법 사례형 연습 교재의 실제 목차(편 → 사례 순서)를 그대로 따라가며, 각 사례를 클릭하면 해당 문항으로 바로 이동합니다.
- **쟁점 검색** — 기존 방식. 쟁점 태그를 검색해서 그 쟁점이 나온 지문 전체를 봅니다.

목차 데이터는 사례집 PDF를 OCR 텍스트로 추출한 뒤 정규식으로 파싱·매칭한 것이라 완벽하지 않습니다. 헌법·행정법 각 142개 사례 중 약 93~94%가 시험(연도/회차)까지 매칭되고, 그중 대부분은 문항(그룹)까지 매칭됩니다. 매칭 안 된 항목은 "미연결"로 표시되고 클릭해도 아무 동작이 없습니다 — 주로 2013년 이전 모의고사(데이터 범위 밖)이거나 OCR 오인식 때문입니다. 문항 단위는 매칭됐지만 그 문항이 여러 설문으로 이뤄진 경우, 그 설문 전체가 아니라 casebook이 가리키는 특정 설문이 아닐 수 있습니다(문항 내부 세부 설문 단위 하이라이트는 아직 지원하지 않음).
