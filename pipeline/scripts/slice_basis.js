...
for (const subj of fs.readdirSync(DATA)) {
  ...
  for (const f of fs.readdirSync(dir)) {
    ...
    else if (exam.casebookAnswers?.length) { src = 'casebook'; res = sliceCasebook(exam); }
    ...
    if (res.ok) {
      groups.forEach((g, gi) => {
        if (src === 'rubric') {
          ...
          g.basisSlice = { src, sig: exam.casebookAnswers.map((x) => (x.answerText || '').length).join(','), idx: res.idx[gi] };
          after += res.kept[gi];
        }
      });
      changed = true;
