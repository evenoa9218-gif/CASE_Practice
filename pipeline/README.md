...
# 민사법 변시 1·2·3회 채점 근거 보강 (정연석 책에 요약만 있는 회차)
python -X utf8 add_cbt_answers.py --write

# 사례집 답안의 뒤바뀐 당사자 기호(乙→江·己, 丙→內·因, 戊→成 …) 되돌리기
python -X utf8 fix_casebook_party.py --write

# 국제법·국제거래법 — 사례집이 없어 두 단계로 끝난다
python -X utf8 build_subject_simple.py 국제법 INT
python -X utf8 build_subject_simple.py 국제거래법 ITL
```
...
**사례집이 없으면** `build_subject_simple.py {과목} {접두사}` 하나로 끝난다. 태깅 결과
(`source/{과목}_사례_final.json`)만 있으면 정제·레지스트리·앱 데이터를 한 번에 만든다.
국제법(INT)·국제거래법(ITL)이 이 길로 들어왔다.

**사례집이 있으면** `형사법` 접미사가 붙은 4개 스크립트를 복사해 과목명·접두사·사례집
파싱 규칙을 바꾼다. 사례집은 책마다 마커 형식이 달라서 `split_casebook_*.py`는 매번 새로
써야 한다(→ [책별 마커 형식](#책별-사례집-마커-형식)).
...
- **채점근거 공백**: 2026-08-11 기준 없다. 형사법 변시 1~5회는 "조균석 책이 QR 온라인 제공으로 바뀌어 못 쓴다"고 적혀 있었으나 재OCR분에 이미 회차당 47k~100k자가 들어와 있었다. 민사법 변시 1·2·3회(정연석 요약 2~3천 자뿐)는 `add_cbt_answers.py`로 박승수 CBT실전답안을 붙여 1.5만~1.6만 자가 됐다. **세 과목 기출 165건 모두 채점 근거가 있다.**
- **사례집 답안의 당사자 기호**: OCR이 `乙`을 `江`·`己`·`心`, `丙`을 `內`·`因`, `戊`를 `成`, 그리고 `乙`을 `Z`·`ZL`로 흘려 놓는다. 형사법 변시 15회 해설은 `乙`과 `丙`이 **한 번도 안 나올** 만큼 심했다. `fix_casebook_party.py`로 578자를 되돌렸다 — 같은 시험의 **문제 지문**을 근거로 삼아, 지문에 없는 기호만 고친다. 숫자 `6`도 `乙`로 자주 읽히지만 일부러 두었다(조사 `도`를 허용하면 판례번호 `2006도2556`이 걸린다).
- **문항 라벨**: 2026-08-11 세 과목 165건 모두 원본 시험지에서 표제를 읽어 확정했다. `relabel_groups.py`의 100점 규칙은 더 이상 쓰지 않는다.

- **선택과목 지문 공백 5건**: 국제법 2014년 3차, 국제거래법 2013년 1·2차·2016년 1·3차는 지문이 없다. 원본이 과목별로 분리되지 않은 "선택과목 사례형" 통합 파일에만 있기 때문이다. 설문·채점기준표는 있어서 앱에는 넣되 목록과 풀이 화면에 「지문 없음」으로 표시한다.
