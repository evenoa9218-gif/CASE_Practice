...
python -X utf8 relabel_civil.py --apply      # 민사법만 다시 — 아래 참고
```

`relabel_groups.py`의 100점 규칙은 **민사법에 성립하지 않는다**(제15회 실제 PDF로 확인:
제1문 150 / 제2문 100 / 제3문 100). 그래서 민사법은 `relabel_civil.py`로 한 번 더 덮어쓴다.
이 스크립트는 원문 표제 → 채점기준표 → PDF 대조 순으로 근거를 찾고, 근거가 없으면
라벨을 만들지 않고 `groupSource: "estimated"`만 남긴다(55건 중 19건 확정 / 36건 추정).
인자 없이 실행하면 미리보기, `--apply`를 붙여야 실제로 쓴다.
...
- **`relabel_groups.py`의 100점 규칙**은 공법 15회 실제 PDF로 검증했고 공법·형사법 110건 중 105건이 정확히 맞는다. **민사법에는 성립하지 않음이 제15회 PDF로 확인됐다**(문당 배점이 150/100/100로 제각각) — `relabel_civil.py`가 대신 처리하며, 근거를 못 찾은 36건은 `groupSource: "estimated"`로 표시된다.
